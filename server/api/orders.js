...
router.put('/addproduct', async (req, res, next) => {
  try {
    ...
    const [ order, create ] = await Order.findOrCreate({ where: { userId: user.id, shippingInfo: user.address, billingInfo: user.address }});
    ...
    const [ orderProducts, created ] = await Order_Products.findOrCreate({ where: { productId: product.id, orderId: orderValues.id } })
